...
| 8. Technical completeness         | Strong automated evidence; independent audit absent  | [`verification-report.md`](verification-report.md) records the full baseline verifier and GASOK candidate verification: Vitest 151/151, Foundry 69/69, the same GIWA flow 7/7 on a public-RPC read-only fork, full Playwright 3/3, typecheck/build, lint, manifests, ABI export, secret scan, dependency audit, security headers, and multi-viewport QA.                                        | Re-run the complete workspace suite at the exact submitted commit. Slither and `git-secrets` were unavailable in the recorded environment, and no independent smart-contract audit has occurred. Add GIWA transaction, receipt, explorer-source, and indexer-reconciliation evidence only after they exist.                           |
...
The program allows multiple tracks, but track selection does not replace
evidence.
