...
| API rate abuse                       | Bounded request body/upload size, in-memory request throttling, pagination, and no unbounded feed DOM. The default is one fail-closed shared bucket; per-IP buckets activate only for an explicitly trusted proxy-rewritten header.                                                                | In-memory throttling is per instance; shared edge limiting is a later production concern. A direct origin must not trust client-supplied forwarding headers.                                                              |
