...
- metadata and social-ownership evidence;
...
| Indexer lag/failure                  | Last-good state is retained; API meta exposes source, block and update time; missing data renders `—` or collection state. Hash-committed metadata is retried with backoff. | Fresh trades can be absent until reconciliation. Receipt success and indexer reconciliation are distinct states.                                                      |
...
| API rate abuse                       | Bounded request body/upload size, in-memory request throttling, pagination, and no unbounded feed DOM.                                                                      | In-memory throttling is per instance; shared edge limiting is a later production concern.                                                                             |
...
| Holder concentration error           | Balances derive from Transfer deltas; pool/locker/vault/burn/zero categories are excluded from ordinary-wallet top ten and shown separately.                                | Rebasing/reflection tokens would break this model, but the factory cannot create them.                                                                                |
